  ...
  describe("Part I - a visit from the past", function() {
    ...
    describe("uniq", function() {
      ...
      it("it should not return a sorted list if isSorted is set to false", function() {
        ...
        const sortedArray = [...Array(100).keys()];

        const notSortedArray = randomArray(1000, 0, 1000);
    ...
    describe("initial", function() {
      ...
      it("should exist", function() {
        expect(_.initial).to.exist;
      });
      it("returns all but the last element", function() {
        expect(_.initial(arra)).to.deep.equal([1,2,3,4,5])
      })
      it("returns all but the last n elements", function() {
        expect(_.initial(arra , 5)).to.deep.equal([])
      })
      it("returns an empty array when n > length", function() {
        expect(_.initial(arra , 7)).to.deep.equal([])
      })
      it("works on an arguments object" , function() {
        var expected = function() {
          return _.initial(arguments)
        }
        expect(expected(1,2,3,4,5,6)).to.deep.equal([1,2,3,4,5])
      })
      it("works well with _.map" , function() {
        var expected = _.map([1,2,3] , function(element) {return element + 2})
        expect(_.initial(expected , 2)).to.deep.equal([3])
      })
    });
    ...
    describe("rest", function() {
      ...
      expect(_.rest(arra)).to.deep.equal([2, 3, 4, 5, 6])
      })
      it("returns the whole array when index is 0" , function() {
        expect(_.rest(arra, 0)).to.deep.equal([1,2,3,4,5,6])
      })
      it("returns elements starting at the given index" , function() {
        expect(_.rest(arra, 1)).to.deep.equal([2,3,4,5,6])
      })
      it("works on an arguments object" , function() {
        var expected = function() {
          return _.rest(arguments)
        }
        expect(expected(1,2,3,4,5,6)).to.deep.equal([2,3,4,5,6])
      })
      it("works well with _.map" , function() {
        var expected = _.map([1,2,3] , function(element) {return element + 2})
        expect(_.rest(expected , 2)).to.deep.equal([5])
      })
    });

    describe("has" , function() {
      var obj = {'name' : 'dylan' , 'age' : 25 , funct : function() {return this.name}}
      it("checks that the object has a property" , function() {
        expect(_.has(obj , 'name')).to.be.true;
      })
      it("returns false if the object doesn't have the property" , function() {
        expect(_.has(obj , 'favoriteFood')).to.be.false;
      })
      it("works for functions too" , function() {
        expect(_.has(obj , 'funct')).to.be.true;
      })
      it("works even when the hasOwnProperty method is deleted." , function() {
        obj = {'name' : 'dylan' , 'age' : 25};
        obj.hasOwnProperty = null;
        //obj.hasOwnProperty('name') ===> you will get null
        expect(_.has(obj , 'name')).to.be.true;
      })
      it("does not check the prototype chain for a property." , function() {
        var obj = {name : 'dylan' , age : 25 , gender: 'male'};
        function Human(namez , age , gender) {this.name = namez ; this.age = age; this.gender = gender};
        Human.prototype = obj;
        var human = new Human('John' , 25 , 'male');
        expect(_.has(human , 'name')).to.be.true;
      })
      it("returns false for null" , function() {
        var obj = {};
        expect(_.has(obj , null)).to.be.false;
      })
      it("returns false for undefined" , function() {
        var obj = {};
        expect(_.has(obj , undefined)).to.be.false;
      })
      it("can check for nested properties" , function() {
        var obj = {a : {b : 'c'}};
        expect(_.has(obj , 'b')).to.be.false; //this is not working properly
      })
      it("does not check the prototype of nested props." , function() {
        ...
        Test.prototype = obj
        var x = new Test;
        expect(_.has(x , 'b')).to.be.false;
      })
    })

    describe("compact" , function() {
      var arra = [1,'','0',0,false,'false',{}]
      it("should return an array with all falsy removed" , function() {
        expect(_.compact(arra)).to.deep.equal([1,'0','false',{}])
      })
      it("works on an arguments object" , function() {
        var expected = function() {
          return _.compact(arguments)
        }
        expect(_.compact(expected(1,'','0',0,false,'false',{}))).to.deep.equal([1,'0','false',{}])
      })
      it("works well with _.map" , function() {
        var expected = function() {
          return _.map(arguments , function(x) {return x + 1}) 
        }
        expect(_.compact(expected(1,2,3))).to.deep.equal([2,3,4])
      })
    })

    describe("without" , function() {
      var arra = [1,2,3,4,5,1,2,3];
      it("removes all instances of the given values" , function() {
        expect(_.without(arra , 5)).to.deep.equal([1,2,3,4,1,2,3])
        expect(_.without(arra , 1,2,3)).to.deep.equal([4,5])
        expect(_.without(arra , 6)).to.deep.equal([1, 2, 3, 4, 5, 1, 2, 3])
      })
      it("works on an arguments object" , function() {
        var expected = function() {
          return _.without(arguments , 3)
        }
        expect(expected(1,2,3)).to.deep.equal([1,2])
      })
      it("compares objects by reference (value case)" , function() {
        var obj = [{one : 1} , {two : 2}]
        expect(_.without(obj , [{one: 1} , {two : 2}])).to.deep.equal([{one : 1} , {two : 2}])
        expect(_.without(obj , obj[0])).to.deep.equal([{two : 2}])
      })
    })

    describe("range", function() {
      it("should return an array" , function() {
        expect(_.range(1,20)).to.be.an('array')
      })
      it("should return the right range" , function() {
        expect(_.range(1,5)).to.deep.equal([1,2,3,4])
        expect(_.range(1,-3,-1)).to.deep.equal([1,0,-1,-2])
        expect(_.range(0,0)).to.deep.equal([])
      }) 
    })
    ...
    describe("find" , function() {
      ...
      it('should find the first element' , function() {
        expect(_.find([1,2,3,4,5] , callback)).to.equal(2)
      })
      it('should return `undefined` if `value` is not found' , function() {
        expect(_.find([1,3,5] , callback)).to.equal(undefined)
      })
      it('iterates array-likes correctly' , function() {
        var expected = [];
        _.each([1,2,3] , function(element) {expected.push(element)})
        expect(expected).to.deep.equal([1,2,3])
      })
      it('works on objects' , function() {
        ...
        expect(_.find(obj , function(element) {return element === 'one'})).to.deep.equal('one')
      })
      it('called with context' , function() {
        ...
        expect(_.find(obj , function(element) {return element === this.contextx} , {contextx : 'one'})).to.deep.equal('one')
      })
    })

    describe("pairs" , function() {
      it("should return the pair values" , function() {
        var obj = {'one' : 1 , 'two' : 2 , 'three' : [3]}
        expect(_.pairs(obj)).to.deep.equal([['one' , 1] , ['two', 2] , ['three' , [3]]])
      }) 
      it("even when one of them is length" , function() {
        var obj1 = {0 : 'one' , 1 : 'two' , 2 : 'three' , length : 3}
        expect(_.pairs(obj1)).to.deep.equal([['0' , 'one'] , ['1' , 'two'] , ['2' , 'three'] , ['length' , 3]])
      })
    })

    describe("object" , function() {
      it("should return an object" , function() {
        var arra1 = ['one','two','three'];
        var arra2 = [1,2,3]
        expect(_.object(arra1,arra2)).to.deep.equal({'one' : 1, 'two' : 2, 'three' : 3})
      })
      it("should return an object with multiple arrays" , function() {
        var arra = [['one' , 1] , ['two' , 2] , ['three' , 3]]
        expect(_.object(arra)).to.deep.equal({'one' : 1, 'two' : 2, 'three' : 3})
      })
      it("an object converted to pairs and back to an object" , function() {
        var pairedobj = _.pairs({0 : 'one' , 1 : 'two' , 2 : 'three'})
        expect(_.object(pairedobj)).to.deep.equal({0 : 'one' , 1 : 'two' , 2 : 'three'})
      })
      it("handles nulls" , function() {
        expect(_.object(null)).to.deep.equal({})
      })
    })
